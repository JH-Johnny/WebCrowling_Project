...
find_index = ["소재지(지번)", "주소(지번)", "재배필지주소(지번)"]
find2_index = ["소재지(도로명)", "주소(도로명)", "재배필지주소(도로명)"]
finded_data = pd.DataFrame()

for i in find_index:
   if i in df.columns:
        finded_data[i] = df[i]
        finded_data[find2_index[find_index.index(i)]] = None
...
for i in finded_data[finded_data.columns[0]]:
    ...
    if not soup.select("div.subject_area span.roadNameText"):
        finded_data[finded_data.columns[1]][idx] = "-"
        idx += 1
        continue
    finded_data[finded_data.columns[1]][idx] = soup.select("div.subject_area span.roadNameText")[0].text # road_name addr
    finded_data[finded_data.columns[0]][idx] = soup.select("div.subject_area span.roadNameText")[1].text.strip().replace("\n", "").replace("\t", "") #lot_number addr
    idx += 1

df[finded_data.columns[1]] = finded_data[finded_data.columns[1]]
df[finded_data.columns[0]] = finded_data[finded_data.columns[0]]
